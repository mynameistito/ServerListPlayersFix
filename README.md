...
The plugin also publishes connected players' SteamID64 values through the Steam server rules interface. Query `A2S_RULES` and read `slpf_steamids_v1`; its value is a JSON array of SteamID64 strings and is refreshed alongside the player data every 5 seconds.
...
SteamID64 values are encoded as strings so consumers such as JavaScript applications do not lose precision.
